...
namespace ImGui
{
    ...
    public:
        ...
        void SetInputBuf(const std::string& str);
...
inline void ImGui::FileBrowser::Display()
{
        ...
        reserveHeight += GetFrameHeightWithSpacing();
    {
        ...
        for(auto &rsc : fileRecords_)
        {
            ...
            if (IsItemClicked(0) && (IsMouseDoubleClicked(0) || io.KeyShift)) {
              if (rsc.isDir) {
                setNewPwd = true;
                newPwd =
...
inline void ImGui::FileBrowser::SetInputBuf(const std::string& str) {
  strncpy(inputNameBuf_->data(), str.c_str(), inputNameBuf_->size());
}
